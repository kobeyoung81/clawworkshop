...
func (r *EventRepository) List(ctx context.Context, params ListEventsParams) ([]models.Event, error) {
	query := r.db.WithContext(ctx).
		Model(&models.Event{}).
		Joins("JOIN workspace_member ON workspace_member.workspace_id = runtime_event.workspace_id AND workspace_member.subject_id = ? AND workspace_member.status = ?", params.SubjectID, "active")
